...
function checkProperty(
  ...
  if (min !== undefined && typeof value === "number" && value < min)
    violations.push({ path: fieldPath, constraint: "minInclusive", value, message: `${msg}: ${value} < ${min}` });

  const max = getNum(store, propNode, "maxInclusive");
  if (max !== undefined && typeof value === "number" && value > max)
    violations.push({ path: fieldPath, constraint: "maxInclusive", value, message: `${msg}: ${value} > ${max}` });

  const minLen = getNum(store, propNode, "minLength");
  if (minLen !== undefined && typeof value === "string" && value.length < minLen)
    violations.push({ path: fieldPath, constraint: "minLength", value: `[len ${value.length}]`, message: `${msg}: length ${value.length} < ${minLen}` });

  const maxLen = getNum(store, propNode, "maxLength");
  if (maxLen !== undefined && typeof value === "string" && value.length > maxLen)
    violations.push({ path: fieldPath, constraint: "maxLength", value: `[len ${value.length}]`, message: `${msg}: length ${value.length} > ${maxLen}` });

  const pattern = getStr(store, propNode, "pattern");
  if (pattern !== undefined && typeof value === "string" && !new RegExp(pattern).test(value))
    violations.push({ path: fieldPath, constraint: "pattern", value: "[string]", message: `${msg}: does not match /${pattern}/` });
}
...
function validateShape(
  ...
  for (const pq of props) {
    ...
    if (maxCount !== undefined && Array.isArray(value) && value.length > maxCount)
      violations.push({ path: fieldPath, constraint: "maxCount", value: `[${value.length}]`, message: `${msg}: too many values` });

    if (value !== undefined) {
      const values = Array.isArray(value) ? value : [value];
      for (const v of values) checkProperty(store, propNode, fieldPath, v, violations);
    }
  }
...
export function loadRegistry(): SchemaRegistry {
  ...
  if (!fs.existsSync(schemaPath)) throw new Error(`REGISTRY_ERR: schema.json not found at ${schemaPath}`);
  ...
  for (const rq of store.getQuads(pad("ContextRegistry"), null, null, null)) {
    ...
    if (nameQuad && shapeQ) contextRegistry.set(nameQuad.object.value, shapeQ.object.value);
  }
  ...
  return { validate, validateSHACL, validateIdentity, authorizedPublicKeys, contextRegistry };
}
